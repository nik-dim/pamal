...
class ExactParetoLP(object):
    ...
    def __init__(self, m, n, r, eps=1e-4):
        ...
        constraints_bal = [self.alpha >= 0, cp.sum(self.alpha) == 1, self.C @ self.alpha >= self.rhs]  # Simplex
        ...
        constraints_rel = [self.alpha >= 0, cp.sum(self.alpha) == 1, self.C @ self.alpha >= 0]  # Relaxed
