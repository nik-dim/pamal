...
class SegNetSplitEncoder(nn.Module):
    def __init__(self, model_type="standard", in_channels=3, rotograd=False):
        super().__init__()
        self.rotograd = rotograd
        # initialise network parameters
    ...
    def forward(self, x):
        ...
        # t3_pred = t3_pred / torch.norm(t3_pred, p=2, dim=1, keepdim=True)
        if self.rotograd:
            return self.flatten(g_decoder[i][1])
...
class SegNetSegmentationDecoder(nn.Module):
    def __init__(self, filters=FILTER, rotograd=False) -> None:
        super().__init__()
        self.rotograd = rotograd
        self.class_nb = 7
        self.pred_task1 = nn.Sequential(
    ...
    def forward(self, x):
        if self.rotograd:
            x = x.view(-1, 64, 128, 256)  # hardcoded
        t1_pred = F.log_softmax(self.pred_task1(x), dim=1)
        return t1_pred
...
class SegNet(nn.Module, BaseCallback):
    ...
    def forward(self, x, return_embedding=False):
        emb = self.segnet(x)
        t1 = self.segmentation_decoder(emb)
        t2 = self.depth_decoder(emb)
        if return_embedding:
            return [t1, t2], emb
        else:
            return [t1, t2]
