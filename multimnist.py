...
def my_app(config: DictConfig) -> None:
    ...
    if config.method.name == "phn":
        ...
        model = SharedBottom(encoder=MultiLeNetR(in_channels=3), decoder=MultiLeNetO(), num_tasks=2)
    elif config.method.name == "rotograd":
        backbone = MultiLeNetR(in_channels=1)
        head1, head2 = MultiLeNetO(), MultiLeNetO()
        model = RotogradWrapper(backbone=backbone, heads=[head1, head2], latent_size=50)
    else:
        model = SharedBottom(encoder=MultiLeNetR(in_channels=1), decoder=MultiLeNetO(), num_tasks=2)
